		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>추대포기수</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>생중량</option>
					<option value="7" ${pager.search == 7 ? 'selected' : ''}>지하부건물중</option>
					<option value="8" ${pager.search == 8 ? 'selected' : ''}>지상부건물중</option>	
					<option value="9" ${pager.search == 9 ? 'selected' : ''}>구직경</option>	
					<option value="10" ${pager.search == 10 ? 'selected' : ''}>초장</option>		
	...
	<table class="table table-striped">
		<thead class="table bg-secondary text-white">
			<tr>
				...
				<th class="order" data-order="bol">추대포기수<c:if test="${pager.order == 'bol'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<th class="order" data-order="udry">지하부건물중<c:if test="${pager.order == 'udry'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<th class="order" data-order="dia">구직경<c:if test="${pager.order == 'dia'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
		...
		<c:if test="${list.size() < 1}">
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						...
						<div id="min">
							...
							<th><input name="items" value="${item.seq2}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.dat}</td>
							<td>${item.area}</td>
							<td>${item.nor}</td>
							...
							<td><a href="delete?seq2=${item.seq2}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq2=${item.seq2}"
