				...
				<tbody>
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq7}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.total}</td>
							<td>${item.basal}</td>
							<td>${item.fer}</td>
							...
							<td><a href="delete?seq7=${item.seq7}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq7=${item.seq7}"
