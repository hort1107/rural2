		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="3" ${pager.search == 3 ? 'selected' : ''}>바이러스병</option>
					<option value="4" ${pager.search == 4 ? 'selected' : ''}>무름병</option>
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>무사마귀병</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>파밤나방</option>
					<option value="7" ${pager.search == 7 ? 'selected' : ''}>담배거세미나방</option>
			...
			<table class="table table-striped">
				<thead class="table bg-secondary text-white">
					<tr>
						...
						<th class="order" data-order="vir">바이러스병<c:if test="${pager.order == 'vir'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="sof">무름병<c:if test="${pager.order == 'sof'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="clu">무사마귀병<c:if test="${pager.order == 'clu'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="bee">파밤나방<c:if test="${pager.order == 'bee'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="lit">담배거세미나방<c:if test="${pager.order == 'lit'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="met">기상재해<c:if test="${pager.order == 'met'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<tbody>
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq6}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.dat}</td>
							<td>${item.vir}</td>
							<td>${item.sof}</td>
							...
							<td><a href="delete?seq6=${item.seq6}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq6=${item.seq6}"
