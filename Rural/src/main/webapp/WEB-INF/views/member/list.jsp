		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="2" ${pager.search == 2 ? 'selected' : ''}>비밀번호</option>
					<option value="3" ${pager.search == 3 ? 'selected' : ''}>이름</option>
					<option value="4" ${pager.search == 4 ? 'selected' : ''}>전화번호</option>
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>직책</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>회사부서</option>	
				</select>
				</div>
				<div class="col-5">
			...
			<table class="table table-striped">
				<thead class="table bg-secondary text-white">
					<tr>
						...
						<th class="order" data-order="id">아이디<c:if test="${pager.order == 'id'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="passwd">비밀번호<c:if test="${pager.order == 'passwd'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="name">이름<c:if test="${pager.order == 'name'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="tel">전화번호<c:if test="${pager.order == 'tel'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="job">직책<c:if test="${pager.order == 'job'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<tbody>
					...
					<c:forEach var="item" items="${list}">
						<tr>
							<th><input name="items" value="${item.id}" type="checkbox" class="form-check-input"> </th>
							<td>${item.id}</td>
							<td>${item.passwd}</td>
							<td>${item.name}</td>
							<td>${item.tel}</td>
