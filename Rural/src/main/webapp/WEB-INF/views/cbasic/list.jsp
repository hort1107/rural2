...
<body>
			...
			<div class="row">
				...
				<div class="col-2">
					<select name="search" class="form-control form-control-sm">
						...
						<option value="6" ${pager.search == 6 ? 'selected' : ''}>정식일
							건물중</option>
		...
		<table class="table table-striped">
			<thead class="table bg-secondary text-white">
				<tr>
					...
					<th class="order" data-order="cul">품종<c:if
							test="${pager.order == 'cul'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th class="order" data-order="lat">위도<c:if
							test="${pager.order == 'lat'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th class="order" data-order="lon">경도<c:if
							test="${pager.order == 'lon'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th class="order" data-order="pla">파종일<c:if
							test="${pager.order == 'pla'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th class="order" data-order="tra">정식일<c:if
							test="${pager.order == 'tra'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th class="order" data-order="dry">정식일건물중<c:if
							test="${pager.order == 'dry'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th class="order" data-order="image">회사부서<c:if
							test="${pager.order == 'image'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th class="order" data-order="dep">사진<c:if
							test="${pager.order == 'dep'}">
							<i
								class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i>
						</c:if></th>
					<th>관리</th>
			...
			<form method="post" action="remove">
				<tbody>
					...
					<c:forEach var="item" items="${list}">
						<c:if
							test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
							<tr>
							<tr data-item="${item.cul}">
								<th><input name="items" value="${item.cul}" type="checkbox"
	...
	<div class="pagination justify-content-center">
		<div>
			<a href="add" class="btn btn-secondary btn-sm mb-2">등록</a> <a
				href=".." class="btn btn-secondary btn-sm mb-2">처음으로</a>
			<button id="remove" class="btn btn-sm btn-outline-danger">
				<i class="bi bi-cart"></i>선택삭제
			</button>
		</div>
	</div>
