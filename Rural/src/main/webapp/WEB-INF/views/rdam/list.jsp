				...
				<tbody>
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq9}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.dat}</td>
							<td>${item.vir}</td>
							<td>${item.sof}</td>
							...
							<td><a href="delete?seq9=${item.seq9}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq9=${item.seq9}"
