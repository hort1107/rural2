		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="4" ${pager.search == 4 ? 'selected' : ''}>포기수</option>
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>개화비율</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>생중량</option>
					<option value="7" ${pager.search == 7 ? 'selected' : ''}>잎건물중</option>
					<option value="8" ${pager.search == 8 ? 'selected' : ''}>줄기건물중</option>	
					<option value="9" ${pager.search == 9 ? 'selected' : ''}>초장</option>	
					<option value="10" ${pager.search == 10 ? 'selected' : ''}>주당착과수</option>		
	...
	<table class="table table-striped">
		<thead class="table bg-secondary text-white">
			<tr>
				...
				<th class="order" data-order="plan">포기수<c:if test="${pager.order == 'plan'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				<th class="order" data-order="per">개화비율<c:if test="${pager.order == 'per'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				<th class="order" data-order="fwe">생중량<c:if test="${pager.order == 'fwe'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				<th class="order" data-order="ldr">잎건물중<c:if test="${pager.order == 'ldr'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				<th class="order" data-order="sdr">줄기건물중<c:if test="${pager.order == 'sdr'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<th class="order" data-order="fru">주당착과수<c:if test="${pager.order == 'fru'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
		...
		<c:if test="${list.size() < 1}">
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<td><a href="delete?seq11=${item.seq11}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq11=${item.seq11}"
