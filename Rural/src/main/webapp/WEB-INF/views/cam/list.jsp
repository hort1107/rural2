	...
	<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="1" ${pager.search == 1 ? 'selected' : ''}>회사부서</option>
				</select>
				</div>
				<div class="col-5">
	...
	<table class="table table-striped">
		...
		<tbody>
					<c:if test="${list.size() < 1}">
						<tr>
							<td colspan="5">등록 작물이 없습니다</td>
						</tr>
					</c:if> 
					<c:forEach var="item" items="${list}">
						<tr>
							<th><input name="items" value="${item.dep}" type="checkbox" class="form-check-input"> </th>
							<td>${item.dep}</td>
							<td><a href="update?dep=${item.dep}" class="btn btn-outline-danger btn-sm">변경</a> <a
								href="delete?dep=${item.dep}" class="btn btn-outline-info btn-sm">삭제</a></td>
						</tr>
					</c:forEach>
					</tbody>
					</form>
