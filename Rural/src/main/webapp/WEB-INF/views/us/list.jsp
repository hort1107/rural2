			...
			<table class="table table-striped">
				<thead class="table bg-secondary text-white">
					<tr>
						...
						<th class="order" data-order="name">제목<c:if test="${pager.order == 'name'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="con">내용<c:if test="${pager.order == 'con'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<tbody>
					...
					<c:forEach var="item" items="${list}">
						<tr>
							...
							<th><input name="items" value="${item.seq14}" type="checkbox" class="form-check-input"> </th>
							<td>${item.name}</td>
							<td>${item.con}</td>
							<td class="toggle">${item.name}</td>
							<td><a href="delete?seq14=${item.seq14}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq14=${item.seq14}"
								class="btn btn-outline-info btn-sm">변경</a></td>
						</tr>
						<tr data-img="${item.name}" class="hide">
