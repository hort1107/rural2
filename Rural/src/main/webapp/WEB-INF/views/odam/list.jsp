		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="3" ${pager.search == 3 ? 'selected' : ''}>노균병</option>
					<option value="4" ${pager.search == 4 ? 'selected' : ''}>검은무늬</option>
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>잎마름</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>흑색썩음 균핵병</option>
					<option value="7" ${pager.search == 7 ? 'selected' : ''}>고자리파리</option>
					<option value="8" ${pager.search == 8 ? 'selected' : ''}>기상재해</option>
					<option value="9" ${pager.search == 9 ? 'selected' : ''}>회사부서</option>					
				</select>
				</div>
				<div class="col-5">
			...
			<table class="table table-striped">
				<thead class="table bg-secondary text-white">
					<tr>
						...
						<th class="order" data-order="dow">노균병<c:if test="${pager.order == 'dow'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="pur">검은무늬<c:if test="${pager.order == 'pur'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="leaf">잎마름<c:if test="${pager.order == 'leaf'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="white">흑색썩음 균핵병<c:if test="${pager.order == 'white'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="delia">고자리파리<c:if test="${pager.order == 'delia'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="meteo">기상재해<c:if test="${pager.order == 'meteo'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<tbody>
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq3}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.dat}</td>
							<td>${item.dow}</td>
							<td>${item.pur}</td>
							...
							<td><a href="delete?seq3=${item.seq3}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq3=${item.seq3}"
