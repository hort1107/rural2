				...
				<tbody>
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq4}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.total}</td>
							<td>${item.basal}</td>
							<td>${item.fer}</td>
							...
							<td><a href="delete?seq4=${item.seq4}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq4=${item.seq4}"
