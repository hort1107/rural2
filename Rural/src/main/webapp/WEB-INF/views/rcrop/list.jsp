		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>지하부생중량</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>지하부건물중</option>
					<option value="7" ${pager.search == 7 ? 'selected' : ''}>지상부건물중</option>
					<option value="8" ${pager.search == 8 ? 'selected' : ''}>초장</option>	
	...
	<table class="table table-striped">
		<thead class="table bg-secondary text-white">
			<tr>
				...
				<th class="order" data-order="fwe">지하부생중량<c:if test="${pager.order == 'fwe'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				<th class="order" data-order="dwe">지하부건물중<c:if test="${pager.order == 'dwe'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				<th class="order" data-order="adwe">지상부건물중<c:if test="${pager.order == 'adwe'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
		...
		<c:if test="${list.size() < 1}">
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq8}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.dat}</td>
							<td>${item.area}</td>
							<td>${item.nor}</td>
							...
							<td><a href="delete?seq8=${item.seq8}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq8=${item.seq8}"
