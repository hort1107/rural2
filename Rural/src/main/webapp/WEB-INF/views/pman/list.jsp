		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="2" ${pager.search == 2 ? 'selected' : ''}>총시비량</option>
					<option value="3" ${pager.search == 3 ? 'selected' : ''}>기비량</option>
					<option value="4" ${pager.search == 4 ? 'selected' : ''}>기비시기</option>
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>추비횟수</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>관개유무</option>
			...
			<table class="table table-striped">
				<thead class="table bg-secondary text-white">
					<tr>
						...
						<th class="order" data-order="total">총시비량<c:if test="${pager.order == 'total'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="basal">기비량<c:if test="${pager.order == 'basal'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="fer">기비시기<c:if test="${pager.order == 'fer'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="dress">추비횟수<c:if test="${pager.order == 'dress'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="irri">관개유무<c:if test="${pager.order == 'irri'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<tbody>
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq10}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.total}</td>
							<td>${item.basal}</td>
							<td>${item.fer}</td>
							...
							<td><a href="delete?seq10=${item.seq10}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq10=${item.seq10}"
