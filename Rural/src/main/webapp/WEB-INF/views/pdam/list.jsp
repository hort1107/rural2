		...
		<div class="mb-2">
			<form>
				...
				<select name="search" class="form-control form-control-sm">
					...
					<option value="3" ${pager.search == 3 ? 'selected' : ''}>역병</option>
					<option value="4" ${pager.search == 4 ? 'selected' : ''}>탄저병</option>
					<option value="5" ${pager.search == 5 ? 'selected' : ''}>담배나방</option>
					<option value="6" ${pager.search == 6 ? 'selected' : ''}>총채벌레류</option>
					<option value="7" ${pager.search == 7 ? 'selected' : ''}>기상재해</option>
					<option value="8" ${pager.search == 8 ? 'selected' : ''}>회사부서</option>				
				</select>
				</div>
				<div class="col-5">
			...
			<table class="table table-striped">
				<thead class="table bg-secondary text-white">
					<tr>
						...
						<th class="order" data-order="phy">역병<c:if test="${pager.order == 'phy'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="ant">탄저병<c:if test="${pager.order == 'ant'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="ori">담배나방<c:if test="${pager.order == 'ori'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
						<th class="order" data-order="thr">총채벌레류<c:if test="${pager.order == 'thr'}"><i class="bi ${pager.orderMode == 1 ? 'bi-caret-up' : pager.orderMode == 2 ? 'bi-caret-down' : ''}"></i></c:if></th>
				...
				<tbody>
					...
					<c:if test="${sessionScope.member.dep == item.dep || sessionScope.member.id == 'admin'}">
						<tr>
							...
							<th><input name="items" value="${item.seq12}" type="checkbox" class="form-check-input"> </th>
							<td>${item.cul}</td>
							<td>${item.dat}</td>
							<td>${item.phy}</td>
							<td>${item.ant}</td>
							...
							<td><a href="delete?seq12=${item.seq12}"
								class="btn btn-outline-danger btn-sm">삭제</a> <a
								href="update?seq12=${item.seq12}"
